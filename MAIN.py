...
class XoLayer(layers.Layer):
    ...
    def call(self, inputs):
        ...
        kernel_ = tf.quantization.fake_quant_with_min_max_args(self.kernel, min=tf.math.reduce_min(self.kernel).numpy(), max=tf.math.reduce_max(self.kernel).numpy(), num_bits=8, narrow_range=False, name=None)
